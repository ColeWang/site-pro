...
export function valueEnumToOptions (valueEnum?: BaseFieldValueEnum): BaseFieldOption[] {
...
export function optionsToValueEnum (options?: BaseFieldOption[]): BaseFieldValueEnum {
    const needOptions: BaseFieldOption[] = compact(options || [])
    // FIX: 只处理一层
    return reduce(needOptions, (result, option) => {
        const { label, value } = option || {}
        if (!(isEmpty(value) || isEmpty(label))) {
            return set(result, value, label)
        }
        return result
    }, {})
}

export function valueEnumToText (text: any, valueEnum?: BaseFieldValueEnum): VNodeChild {
    const needValueEnum: BaseFieldValueEnum = omitNil(valueEnum)
    // VNode
    ...
    if (isArray(text)) {
        const children: VNodeChild[] = compact(text).map((value) => {
            return valueEnumToText(value, needValueEnum)
        })
